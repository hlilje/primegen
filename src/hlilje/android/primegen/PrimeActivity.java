...
public class PrimeActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (intent != null) {
			...
			String warning = intent
					.getStringExtra(MainActivity.WARNING_MESSAGE);
			if (warning != null)
				view.setText(warning); // Set warning and don't do anything else
			else {
				// The output of the primeGenerator as a String meant to be
				// printed
	...
	/**
	 * Method responsible for generating the prime numbers to display in the
	 * PrimeActivity. Will generate all prime numbers from 1 to the limit
	 * specified as a parameter (exclusive). This method is extremely
	 * inefficient and should be treated as such.
	 * 
	 * @param limit
	 *            the limit to which all primes should be generated
	...
	private String primeGenerator(int limit) {
		...
		for (int i = 2; i < limit; i++) {
			...
			/*
			 * Check if the number is divisible by 2 or any other number smaller
			 * than the current number. If that is the case, break the loop
			 * since the number isn't a prime number.
			 */
			for (int j = 2; j < i; j++) {
				if (i % j == 0) {
					isPrime = false;
